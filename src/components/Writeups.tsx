...
export const Writeups = () => {
  ...
  const updateURL = (directory: string, tag: string) => {
    ...
    navigate(`/writeups${search ? `?${search}` : ""}`, { replace: true });
  };
  ...
  useEffect(() => {
    const fetchPosts = async () => {
      ...
      const posts = Object.entries(postModules).map(([path, content]) => {
        ...
        const relativePath = path.replace("../writeups/", "").replace(".md", "");
        const filename =
  ...
  if (loading) {
    ...
    return (
      <div className="w-full px-4 sm:px-8">
        <SEO 
          title={`Writeups${selectedDirectory !== "all" ? ` - ${selectedDirectory}` : ""}${selectedTag !== "all" ? ` #${selectedTag}` : ""} - tlsbollei`}
